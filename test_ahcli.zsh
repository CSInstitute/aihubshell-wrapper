#!/usr/bin/env zsh
#
# test_ahcli.zsh — ahcli.zsh 안전 테스트 하니스 (zsh 버전)
#
# 설계 원칙 (SAFETY):
...
#      AIHUB_CONF / HOME 을 샌드박스로 격리한다. ~/.config, ~/.zshrc 안 건드림.
#   3. 위험 명령(install: /opt 복사 + rc 수정) 은 실행하지 않고 정적 검사만.
#   4. get/pget 의 실제 다운로드는 절대 수행하지 않음 (mock 이 가로챔).
#
# 옵션:
#   RUN_NETWORK=1  ./test_ahcli.zsh   # ls/pls 의 읽기 전용 실제 API 호출까지 포함
#
set -uo pipefail

SCRIPT_DIR="${0:A:h}"
WRAPPER="$SCRIPT_DIR/ahcli.zsh"
...
# 이 하니스의 install/uninstall 검증은 호스트 셸을 zsh 로 고정해 결정론적으로 한다
# (rc 타깃=~/.zshrc). 셸별 감지 동작 자체는 [11] 에서 $SHELL 주입으로 따로 검증.
...
run() {
  ...
  zsh "$PREFIX/ahcli" "$@" >"$SANDBOX/_out" 2>&1
  RC=$?
  OUT="$(cat "$SANDBOX/_out")"
}

echo "════════════════════════════════════════════"
echo " ahcli.zsh 안전 테스트"
...
# ───────────────────────────────────────────────
if zsh -n "$WRAPPER" 2>/tmp/synerr; then ok "zsh -n 구문 검사 통과"
else bad "zsh -n 구문 검사" "$(cat /tmp/synerr)"; fi

# shellcheck 은 zsh 를 지원하지 않으므로 건너뜀
echo "  $(c_dim '· shellcheck zsh 미지원 — 건너뜀')"
...
out="$(
  ...
  zsh "$PREFIX/ahcli" get 71265 2>&1
)"
...
cp "$WRAPPER" "$NOSRC/ahcli.zsh"
out="$(
  HOME="$FAKE_HOME" AIHUB_PREFIX="$SANDBOX/prefix_neg" \
  zsh "$NOSRC/ahcli.zsh" install 2>&1
)"; rc_inst_neg=$?
...
out="$(
  ...
  zsh "$SRCDIR/ahcli.zsh" install 2>&1
)"; rc_inst=$?
...
HOME="$UHOME" AIHUB_PREFIX="$UPREFIX" zsh "$SRCDIR/ahcli.zsh" install >/dev/null 2>&1
...
out="$(
  ...
  zsh "$SRCDIR/ahcli.zsh" uninstall 2>&1
)"; rc_uninst=$?
...
if ! grep -qF "# aihub" "$UHOME/.zshrc"; then ok "uninstall: rc 의 PATH 라인 제거"
else bad "uninstall: rc 의 PATH 라인 제거" "# aihub 라인 잔존"; fi
if grep -qx "echo userline" "$UHOME/.zshrc"; then ok "uninstall: rc 의 사용자 줄 보존"
...
rc_targets() {  # $1=SHELL $2=HOME [$3=ZDOTDIR] [$4=ENV] — 빈 값은 unset 과 동일
  SHELL="$1" HOME="$2" ZDOTDIR="${3:-}" ENV="${4:-}" zsh "$WRAPPER" __rc-targets 2>/dev/null
}
host_shell() { SHELL="$1" HOME="$2" zsh "$WRAPPER" __host-shell 2>/dev/null; }
