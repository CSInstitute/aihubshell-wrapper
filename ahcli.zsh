#!/usr/bin/env zsh
# ahcli — aihubshell 래퍼 (zsh 버전)

# --- 실행 셸 가드: zsh 가 아니면 중단 (set -o pipefail 보다 먼저 검사) ---
if [ -z "${ZSH_VERSION:-}" ]; then
  echo "ahcli.zsh requires zsh. Use ahcli.bash (bash) or ahcli.sh (POSIX sh)." >&2
  exit 1
fi
...
_host_shell() {
  local hs="${SHELL:-}"
  if [[ -z "$hs" ]] && command -v getent >/dev/null 2>&1; then
    hs="$(getent passwd "$(id -un)" 2>/dev/null | cut -d: -f7 || true)"
  fi
  if [[ -z "$hs" ]] && command -v dscl >/dev/null 2>&1; then
    hs="$(dscl . -read "/Users/$(id -un)" UserShell 2>/dev/null | awk '{print $2}' || true)"
  fi
  basename "${hs:-/bin/sh}"
}
...
_rc_cleanup_candidates() {
  local zdir="${ZDOTDIR:-$HOME}"
  {
    printf '%s\n' \
      ...
      "$zdir/.zshrc" "$zdir/.zprofile" "$zdir/.zshenv"
...
case "$cmd" in
  ...
  ls)
    _ensure_shell
    # zsh 는 unquoted 확장을 단어 분리하지 않으므로 배열로 인자를 구성
    extra=(); [[ -n "${1:-}" ]] && extra=(-datasetkey "$1")
    aihubshell -mode l "${extra[@]}" ;;

  pls)
    _ensure_shell
    extra=(); [[ -n "${1:-}" ]] && extra=(-datapckagekey "$1")
    aihubshell -mode pl "${extra[@]}" ;;
