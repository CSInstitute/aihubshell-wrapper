...
# 이 하니스의 install/uninstall 검증은 호스트 셸을 bash 로 고정해 결정론적으로 한다
# (rc 타깃=~/.bashrc). 셸별 감지 동작 자체는 [11] 에서 $SHELL 주입으로 따로 검증.
...
( umask 077; HOME="$FAKE_HOME" AIHUB_PREFIX="$UMP" bash "$SRCDIR/ahcli.bash" install >/dev/null 2>&1 )
...
rc_targets() {  # $1=SHELL $2=HOME [$3=ZDOTDIR] [$4=ENV] — 빈 값은 unset 과 동일
  SHELL="$1" HOME="$2" ZDOTDIR="${3:-}" ENV="${4:-}" bash "$WRAPPER" __rc-targets 2>/dev/null
}
host_shell() { SHELL="$1" HOME="$2" bash "$WRAPPER" __host-shell 2>/dev/null; }
